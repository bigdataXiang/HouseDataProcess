 ...
 */
	...
	public static void fileCut(int n, String folder) {
		try {
			File file = new File(folder + "ResultLean.txt");
			FileInputStream fis = new FileInputStream(file);
			InputStreamReader isr = new InputStreamReader(fis, "UTF-8");
			BufferedReader reader = null;
			String rs = null;
			...
			while ((rs = reader.readLine()) != null) {
				count++;
				if (count < 120000) {
					FileTool.Dump(rs, folder + "ResultLean_OK_1.txt", "utf-8");

				} else {
					FileTool.Dump(rs, folder + "ResultLean_OK_2.txt", "utf-8");
				}
			}
